package com.example.joyrun.runpage.view
...
import com.example.joyrun.runpage.viewmodel.RunIndoorViewModel
import com.example.joyrun.runpage.viewmodel.RunIndoorViewModelFactory
...
class RunIndoorActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val targetDistance = intent.getFloatExtra("distance", 0f)
        binding.targetDistance.setText(String.format("目标距离: %.2f 公里", targetDistance))
        ...
        viewModel.runningEvent.observe(this) {
            ...
            if (targetDistance >= it.totalDistance / 1000f) {
                binding.tvIndoorRemain.text =
                    "%.2f".format(targetDistance - it.totalDistance / 1000f)

                val percent = (it.totalDistance / (targetDistance * 1000f) * 100).toInt()
                binding.barIndoor.progress = percent
                binding.tvIndoorPercent.text = "已完成 $percent%"
            } else {
                binding.tvIndoorPercent.text = "已完成 100%"
            }
        }
